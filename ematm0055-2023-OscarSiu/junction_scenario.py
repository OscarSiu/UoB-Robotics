...
# Description: Multiple vehicles Junction Scenario
import glob
import os
import sys
...
def spawn_ego_vehicle(world):
    ...
    ego_spawnpoint = world.get_map().get_spawn_points()[46] # TODO: Change spawn point
...
def spawn_npcs(world, vehicle_spawnpoint):   
    npc_sp = [241, 240, 242, 239, 47, 238] 
    npc_bp = random.choice(bp_lib.filter('vehicle.audi.*'))
    for i in npc_sp:
        npc_spawnpoint = world.get_map().get_spawn_points()[i]
        npc = world.spawn_actor(npc_bp, npc_spawnpoint)
        #vehicle.set_autopilot(True, 8000)
        
    v1 = world.spawn_actor(npc_bp, carla.Transform(carla.Location(
        vehicle_spawnpoint.location.x+50, vehicle_spawnpoint.location.y-5, 
        vehicle_spawnpoint.location.z)))

    v2 = world.spawn_actor(npc_bp, carla.Transform(carla.Location(
        vehicle_spawnpoint.location.x+200, vehicle_spawnpoint.location.y, 
        vehicle_spawnpoint.location.z)))
        
    v3 = world.spawn_actor(npc_bp, carla.Transform(carla.Location(
        vehicle_spawnpoint.location.x+30, vehicle_spawnpoint.location.y+5, 
        vehicle_spawnpoint.location.z)))
...
def attach_sensors(world, vehicle):
    ...
    radar_bp.set_attribute('vertical_fov', '25')
...
def main():
    ...
    try:
        ...
        ego_vehicle.apply_control(carla.VehicleControl(throttle=0.7, brake=0.0))


        spawn_npcs(world, ego_spawnpoint)

        # Destination = ego_spawnpoint
        # Destination.location.x +=200
        Destination = world.get_map().get_spawn_points()[237]
