//~/carla/Unreal/CarlaUE4/Plugins/Carla/Source/Carla/Actor/ActorBlueprintFunctionLibrary.cpp
...
//CUSTOM SENSOR calyopulse'
FActorDefinition UActorBlueprintFunctionLibrary::MakeCalyoPulseDefinition()
{
  FActorDefinition Definition;
  bool Success;
  MakeCalyoPulseDefinition(Success, Definition);
  check(Success);
  return Definition;
}

void UActorBlueprintFunctionLibrary::MakeCalyoPulseDefinition(
    bool &Success,
    FActorDefinition &Definition)
{
  FillIdAndTags(Definition, TEXT("sensor"), TEXT("other"), TEXT("calyopulse"));
  AddVariationsForSensor(Definition);

  FActorVariation HorizontalFOV;
  HorizontalFOV.Id = TEXT("horizontal_fov");
  HorizontalFOV.Type = EActorAttributeType::Float;
  HorizontalFOV.RecommendedValues = { TEXT("120") };
  HorizontalFOV.bRestrictToRecommended = false;

  FActorVariation VerticalFOV;
  VerticalFOV.Id = TEXT("vertical_fov");
  VerticalFOV.Type = EActorAttributeType::Float;
  VerticalFOV.RecommendedValues = { TEXT("120") };
  VerticalFOV.bRestrictToRecommended = false;
  ...
  Range.Id = TEXT("range");
  Range.Type = EActorAttributeType::Float;
  Range.RecommendedValues = { TEXT("10") }; // [meters]
  Range.bRestrictToRecommended = false;
  ...
  PointsPerSecond.Id = TEXT("points_per_second");
  PointsPerSecond.Type = EActorAttributeType::Int;
  PointsPerSecond.RecommendedValues = { TEXT("8000") };
  PointsPerSecond.bRestrictToRecommended = false;
  ...
  AtmospAttenRate.Id = TEXT("atmosphere_attenuation_rate");
  AtmospAttenRate.Type = EActorAttributeType::Float;
  AtmospAttenRate.RecommendedValues = { TEXT("0.208") };
...
// CUSTOM CALYO PULSE SENSOR
void UActorBlueprintFunctionLibrary::SetCalyoPulse(
    const FActorDescription &Description,
    ACalyoPulse *CalyoPulse)
{
  CARLA_ABFL_CHECK_ACTOR(CalyoPulse);
  constexpr float TO_CENTIMETERS = 1e2;
  ...
  {
    CalyoPulse->SetSeed(CalyoPulse->GetRandomEngine()->GenerateRandomSeed());
  }

  CalyoPulse->SetHorizontalFOV(
      RetrieveActorAttributeToFloat("horizontal_fov", Description.Variations, 120.0f));
  CalyoPulse->SetVerticalFOV(
      RetrieveActorAttributeToFloat("vertical_fov", Description.Variations, 45.0f));
  CalyoPulse->SetRange(
      RetrieveActorAttributeToFloat("range", Description.Variations, 15.0f) * TO_CENTIMETERS);
  CalyoPulse->SetPointsPerSecond(
      RetrieveActorAttributeToInt("points_per_second", Description.Variations, 8000));

  CalyoPulse->SetAtmosphereAttenuationRate(
      RetrieveActorAttributeToFloat("atmosphere_attenuation_rate", Description.Variations, 0.004f));
