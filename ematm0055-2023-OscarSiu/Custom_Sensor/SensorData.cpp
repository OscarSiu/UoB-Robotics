...
//~/carla/PythonAPI/carla/source/libcarla/SensorData.cpp
...
//OSCAR CUSTOMIZATION
  std::ostream &operator<<(std::ostream &out, const CalyoPulseMeasurement &meas) {
    out << "CalyoPulseMeasurement(frame=" << std::to_string(meas.GetFrame())
        << ", timestamp=" << std::to_string(meas.GetTimestamp())
        << ", point_count=" << std::to_string(meas.GetDetectionAmount())
        //<< ", number_of_points=" << std::to_string(meas.size())
        << ')';
    return out;
      ...
      std::ostream &operator<<(std::ostream &out, const CalyoPulseDetection &det) {
    out << "CalyoPulseDetection(velocity=" << std::to_string(det.velocity)
        << ", azimuth=" << std::to_string(det.azimuth)
        << ", altitude=" << std::to_string(det.altitude)
        << ", depth=" << std::to_string(det.depth)
        // << ", intensity=" << std::to_string(det.intensity)
...
void export_sensor_data() {
  ...
  class_<csd::CalyoPulseMeasurement, bases<cs::SensorData>, boost::noncopyable, boost::shared_ptr<csd::CalyoPulseMeasurement>>("CalyoPulseMeasurement", no_init)
    .add_property("raw_data", &GetRawDataAsBuffer<csd::CalyoPulseMeasurement>)
    .def("get_detection_count", &csd::CalyoPulseMeasurement::GetDetectionAmount)
    .def("__len__", &csd::CalyoPulseMeasurement::size)
    .def("__iter__", iterator<csd::CalyoPulseMeasurement>())
    .def("__getitem__", +[](const csd::CalyoPulseMeasurement &self, size_t pos) -> csd::CalyoPulseDetection {
      return self.at(pos);
    })
    .def("__setitem__", +[](csd::CalyoPulseMeasurement &self, size_t pos, const csd::CalyoPulseDetection &detection) {
      self.at(pos) = detection;
    })
    .def(self_ns::str(self_ns::self))
  ;
  ...
  class_<csd::CalyoPulseDetection>("CalyoPulseDetection")
    .def_readwrite("velocity", &csd::CalyoPulseDetection::velocity)
    .def_readwrite("azimuth", &csd::CalyoPulseDetection::azimuth)
    .def_readwrite("altitude", &csd::CalyoPulseDetection::altitude)
    .def_readwrite("depth", &csd::CalyoPulseDetection::depth)
    .def_readwrite("intensity", &csd::CalyoPulseDetection::intensity)
